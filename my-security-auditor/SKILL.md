...
| Network / infrastructure access (node shell, kubectl, cloud API) | `references/frameworks/network-security-audit.md` — namespace access (Linux netns + K8s), network services inventory, traffic flow journeys (pod-to-pod, pod-to-external, node-to-node, site-to-site VPN, client-to-site VPN + ZTNA, user-to-app), NetworkPolicy auditing (default-deny, DNS egress, CNI extensions, service mesh L7), host firewall auditing (iptables, nftables, firewalld, UFW, pf, Windows, cloud SGs/NACLs), edge auditing (WAF: AWS/Cloudflare/Akamai/Fastly/F5 ASM/ModSecurity+CRS with direct-to-origin bypass testing; load balancers: F5 BIG-IP, Citrix ADC, HAProxy, NGINX, ingress-nginx with TLS/XFF/CVE audit; API gateways: Kong, AWS API Gateway, Apigee; reverse proxy smuggling + cache poisoning) |
...
3. **Traffic flow journeys** — apply the nine-step methodology to significant paths: pod-to-pod same-ns, pod-to-pod cross-ns, pod-to-external (metadata service!), node-to-node control plane (kubelet 10250, kube-apiserver 6443, etcd 2379/2380, overlay VXLAN 8472/Geneve 6081, BGP 179), site-to-site VPN (IPsec `ip xfrm`, WireGuard `wg show`, OpenVPN config), client-to-site VPN (concentrator auth + MFA + device posture + split/full tunnel + DNS leak check + kill switch + offboarding SLA; known mass-exploited CVEs in Pulse/Ivanti/Citrix/Fortinet/Cisco ASA/SonicWall/GlobalProtect; ZTNA platforms as modern replacement), user-to-application (DNS → CDN → LB → ingress → pod; check TLS termination points and back-end mTLS). **Also audit the edge layer itself** (section 7 of the reference): WAF configuration and bypass testing (direct-to-origin is the #1 WAF finding), load balancer TLS/XFF/health-check hygiene, API gateway auth + rate limiting, reverse proxy smuggling and cache poisoning, mass-exploited edge CVEs (F5, Citrix, ingress-nginx IngressNightmare).
...
6. **WAF, load balancer, API gateway, reverse proxy auditing** — edge architecture inventory (CDN → WAF → LB → ingress → pod); WAF products (AWS WAF, Cloudflare, Akamai, Fastly/Signal Sciences, F5 ASM, Fortinet FortiWeb, ModSecurity + OWASP CRS); direct-to-origin bypass testing (one of the highest-impact edge findings); WAF mode (prevention vs detection-only), managed rule group coverage and version lag, body inspection limits, HTTP/2/3 + WebSocket + gRPC coverage, bypass testing (gotestwaf, nowafpls); load balancer TLS config (testssl.sh / sslyze), certificate lifecycle, health check hygiene, sticky session cookie flags, X-Forwarded-For chain audit (attacker-spoofable IP if misconfigured); F5 BIG-IP (tmsh, iRules, TMUI hardening), Citrix ADC/NetScaler (NSIP internal-only), NGINX/HAProxy config review, ingress-nginx snippet annotation risk; API gateway audit (JWT algorithm pinning, OAuth scope granularity, Kong admin API internal-only, AWS API Gateway resource policies); reverse proxy bugs (HTTP request smuggling CL.TE/TE.CL/TE.TE, h2↔h1 desync, host header confusion, path traversal, CRLF injection, cache poisoning via unkeyed headers); TLS termination topology mapping; mass-exploited edge CVEs to cross-check patch level (F5 CVE-2020-5902/2022-1388/2023-46747, Citrix CVE-2019-19781/2023-3519/CitrixBleed 2023-4966, ingress-nginx IngressNightmare CVE-2025-24513/1097/1098/24514/1974, HAProxy CVE-2021-40346, Apache CVE-2021-41773/42013, Kong CVE-2024-32876).
